        ...
        <div className="p-3">
          <div className="rounded-xl bg-[#002724] p-3.5 border border-[#0d4742]/50">
            ...
            {(!modelStatus || modelStatus.error) ? (
              ...
              <div className="mt-1 flex items-center gap-1.5 text-xs text-white">
                <span className="h-2 w-2 rounded-full bg-red-500" />
                <span className="font-medium">Models unavailable</span>
              </div>
            ) : (
              <>
                <div className="mt-1 flex items-center gap-1.5 text-xs text-white">
                  <span className="h-2 w-2 rounded-full bg-[#10b981]" />
                  <span className="font-medium">Models operational</span>
                </div>
                <p className="mt-0.5 text-[11px] text-[#6d8a86]">
                  NLP v{modelStatus.pipeline_version || '?.?'} · Layers: {[
                    modelStatus.layers?.rules && 'rules',
                    modelStatus.layers?.gliner && 'gliner',
                    modelStatus.layers?.heads && 'heads'
                  ].filter(Boolean).join(', ')}
                </p>
              </>
            )}
